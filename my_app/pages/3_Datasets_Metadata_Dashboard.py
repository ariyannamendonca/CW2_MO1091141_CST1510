...
with tab_dashboard:
    ...
    if submitted:
        dataset_id = datasets.insert_dataset(name, category, uploaded_by, uploaded_date, rows, columns, file_size_mb, created_at)
    ...
    dataset_ids = df['dataset_id'].tolist() if not df.empty and 'dataset_id' in df.columns else []
    ...
    with st.form("update_form", clear_on_submit=True):
        ...
        if update_submitted:
            if upd_id:
                rows_affected =datasets.update_dataset_rows_columns(upd_id, upd_rows, upd_columns)
    ...
    if del_submit and del_id:
        rows_deleted = datasets.delete_dataset(del_id)
        if rows:
    ...
    if df.empty:
        ...
        uploader_counts.columns = ['uploaded by', 'Count']
        st.bar_chart(uploader_counts.set_index('uploaded by'))
        st.divider()
